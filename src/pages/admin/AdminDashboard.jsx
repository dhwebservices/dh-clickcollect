...
export default function AdminDashboard() {
  ...
  async function load() {
    ...
    try {
      ...
      setRestaurants(Array.isArray(restaurantRows) ? restaurantRows.filter(Boolean) : [])
      setOrders(Array.isArray(orderRows) ? orderRows.filter(Boolean) : [])
    } catch {
      setRestaurants([])
      setOrders([])
    } finally {
  ...
  const pending = safeOrders.filter((order) => order?.status === 'pending').length
  ...
  return (
    <div style={{ display: 'grid', gap: 24 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 14 }}>
        <StatCard icon={Store} label="Restaurants" value={safeRestaurants.length} sub={`${safeRestaurants.filter((item) => item?.status === 'active').length} active`} />
        <StatCard icon={ShoppingBag} label="Recent orders" value={safeOrders.length} sub={`${pending} pending right now`} />
        <StatCard icon={TrendingUp} label="Paid volume" value={`£${gross.toFixed(2)}`} sub={`${paidOrders.length} paid orders`} />
        <StatCard icon={Clock4} label="Suspended" value={safeRestaurants.filter((item) => item?.status === 'suspended').length} sub="Needs follow-up" />
      </div>
          ...
          {loading ? (
            ...
            >
              ...
              <div style={{ textAlign: 'right' }}>
                <div style={{ color: status === 'active' ? '#86efac' : status === 'pending' ? '#fcd34d' : '#fca5a5', fontSize: 13 }}>
                  {status}
                </div>
          ...
          {loading ? (
            ...
            <div key={order.id} style={{ padding: '14px 20px', borderTop: '1px solid #111', display: 'flex', justifyContent: 'space-between', gap: 16 }}>
              ...
              <div style={{ textAlign: 'right' }}>
                ...
                <div style={{ color: '#666', fontSize: 12, marginTop: 4 }}>{order.status || 'unknown'}</div>
              </div>
            </div>
          ))}
        </section>
